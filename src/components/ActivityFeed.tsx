...
import { Task, PROJECTS, TEAM_MEMBERS } from '@/types/task';
...
interface Activity {
  id: string;
  type: 'created' | 'moved' | 'assigned' | 'completed' | 'progress' | 'status' | 'delayed' | 'critical';
  task: Task;
  timestamp: string;
  message: string;
  actor?: string;
...
export function ActivityFeed({ tasks }: ActivityFeedProps) {
  ...
  const getProjectEmoji = (task: Task) => {
    const project = PROJECTS.find(p => p.id === task.project);
    return project?.icon || '📋';
  };
